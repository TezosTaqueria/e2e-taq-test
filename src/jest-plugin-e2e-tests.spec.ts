...
describe('Jest Plugin E2E Testing for the Taqueria CLI', () => {
	...
	test('test will create a default "tests" partition and jest config when called with no arguments', async () => {
		...
		const { stdout } = await execute('taq', 'install @taqueria/plugin-jest@0.25.23-rc', './test-project');
	...
	test('test can be run successfully with jest plugin', async () => {
		...
		const test_file = await (await exec(`cat src/test-data/empty-jest-test-file-1.ts`)).stdout;
	...
	test('all tests inside of a test partition will run', async () => {
		...
		const test_file_one = await (await exec(`cat src/test-data/empty-jest-test-file-1.ts`)).stdout;
		await writeFile('./test-project/multi-file-test/empty-jest-test-file-1.spec.ts', test_file_one);
		const test_file_two = await (await exec(`cat src/test-data/empty-jest-test-file-2.ts`)).stdout;
		...
		expect(stderr).toEqual(expect.arrayContaining([expect.stringContaining('PASS  multi-file-test/empty-jest-test-file-1.spec.ts')]));
		expect(stderr).toEqual(expect.arrayContaining([expect.stringContaining('PASS  multi-file-test/empty-jest-test-file-2.spec.ts')]));
	...
	test('create contract-test will create an integration-test for a contract from test stubs', async () => {
		...
		const tz_file = await (await exec(`cat src/test-data/increment.tz`)).stdout;
