...
describe('Metadata Plugin E2E Testing for the Taqueria CLI', () => {
	...
	test('1652 - generate-project-metadata will add a metadata entry to the config.json', async () => {
		...
		const { stdout } = await execute('taq', 'install @taqueria/plugin-metadata@0.25.31-rc', './test-project');
	...
	test('1652 - generate-metadata will create a metadata json file in the artifacts directory', async () => {
		...
		const { stdout: stdout1 } = await execute('taq', 'install @taqueria/plugin-metadata@0.25.31-rc', './test-project');
