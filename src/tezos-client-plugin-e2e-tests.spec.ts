...
describe('Tezos-Client Plugin E2E Testing for Taqueria CLI', () => {
	...
	test('typecheck will check a contract in the artifacts folder', async () => {
		...
		const { stdout } = await execute('taq', 'install @taqueria/plugin-tezos-client@0.26.28-rc', './test-project');
		expect(stdout).toContain('Plugin installed successfully');

		const artifact_file = await (await exec(`cat src/test-data/hello-tacos.tz`)).stdout;
		await writeFile('./test-project/artifacts/hello-tacos.tz', artifact_file);

		const { stdout: stdout1, stderr } = await execute('taq', 'typecheck', './test-project');
		console.log(stderr)
	...
	test('typecheck will check one contract using typecheck [sourceFile] command', async () => {
		const { execute, cleanup, spawn, writeFile, exists, readFile } = await prepareEnvironment();
		const { waitForText } = await spawn('taq', 'init test');
		await waitForText("Project taq'ified!");
		await execute('taq', 'install @taqueria/plugin-core@0.26.28-rc', './test');
		const { stdout } = await execute('taq', 'install @taqueria/plugin-tezos-client@0.26.28-rc', './test');
		expect(stdout).toContain('Plugin installed successfully');

		await new Promise(r => setTimeout(r, 3000));
		
		const artifact_file = await (await exec(`cat src/test-data/hello-tacos.tz`)).stdout;
		await writeFile('./test/artifacts/hello-tacos.tz', artifact_file);
		console.log(await readFile('./test/artifacts/hello-tacos.tz'))
		await exists('./test/artifacts/hello-tacos.tz')

		const { stdout: stdout1, stderr } = await execute('taq', 'typecheck hello-tacos.tz', './test');
		console.log(stderr)
		expect(stdout1).toEqual(expect.arrayContaining(['│ hello-tacos.tz │ Valid  │']));
	...
	test('typechecker will check all contracts under artifacts folder', async () => {
		...
		const artifact_one = await (await exec(`cat src/test-data/hello-tacos.tz`)).stdout;
		await writeFile('./test-project/artifacts/hello-tacos-one.tz', artifact_one);
		const artifact_two = await (await exec(`cat src/test-data/hello-tacos.tz`)).stdout;
	...
	// blocked by https://github.com/ecadlabs/taqueria/issues/1671
	test('typecheck willcheck multiple (but not all) contracts using typecheck [sourceFile1] [sourceFile2] command', async () => {
		const { execute, cleanup, spawn, writeFile } = await prepareEnvironment();
		const { waitForText } = await spawn('taq', 'init test-project');
		await waitForText("Project taq'ified!");
		const { stdout } = await execute('taq', 'install @taqueria/plugin-tezos-client@0.26.28-rc', './test-project');
		...
		const artifact_two = await (await exec(`cat src/test-data/hello-tacos.tz`)).stdout;
		await writeFile('./test-project/artifacts/hello-tacos-two.tz', artifact_two);
		const artifact_three = await (await exec(`cat src/test-data/hello-tacos.tz`)).stdout;
	...
	test('different tezos client image can be used', async () => {
	...
	test('typecheck will error and yet displays table if contract is ill-typed', async () => {
		...
		const artifact_file = await (await exec(`cat src/test-data/hello-tacos-ill-typed.tz`)).stdout;
		await writeFile('./test-project/artifacts/hello-tacos-ill-typed.tz', artifact_file);
		await writeFile('./project/artifacts/hello-tacos-ill-typed.tz', artifact_file);
	...
	test('simulate will simulate one contract using simulate [sourceFile] command', async () => {
		...
		const artifact_two = await (await exec(`cat src/test-data/integerParameter10.tz`)).stdout;
		await writeFile('./test-project/artifacts/integerParameter10.tz', artifact_two);
		const artifact_three = await (await exec(`cat src/test-data/anyContract.storage.tz`)).stdout;
	...
	test('simulate will pass errors from Michelson', async () => {
		...
		const { waitForText } = await spawn('taq', 'init test-project');
		await waitForText("Project taq'ified!");
		await execute('taq', 'install @taqueria/plugin-core@0.26.28-rc', './test-project');
		const { stdout } = await execute('taq', 'install @taqueria/plugin-tezos-client@0.26.28-rc', './test-project');
		expect(stdout).toContain('Plugin installed successfully');

		const artifact_one = await (await exec(`cat src/test-data/hello-tacos.tz`)).stdout;
		await writeFile('./test-project/artifacts/hello-tacos.tz', artifact_one);
		const artifact_two = await (await exec(`cat src/test-data/integerParameter15.tz`)).stdout;
		await writeFile('./test-project/artifacts/integerParameter15.tz', artifact_two);
		const artifact_three = await (await exec(`cat src/test-data/anyContract.storage.tz`)).stdout;
		await writeFile('./test-project/artifacts/anyContract.storage.tz', artifact_three);
		await exists(`./test-project/artifacts/anyContract.storage.tz`);
	...
	test('simulate will error with incorrect parameter', async () => {
		...
		const artifact_one = await (await exec(`cat src/test-data/hello-tacos.tz`)).stdout;
		await writeFile('./test-project/artifacts/hello-tacos.tz', artifact_one);
		const artifact_two = await (await exec(`cat src/test-data/stringParameter.tz`)).stdout;
	...
	test('simulate will error on supplying list instead of nat', async () => {
		...
		const artifact_one = await (await exec(`cat src/test-data/hello-tacos.tz`)).stdout;
		await writeFile('./test-project/artifacts/hello-tacos.tz', artifact_one);
		const artifact_two = await (await exec(`cat src/test-data/listParameter.tz`)).stdout;
	...
	test('simulate will error when map supplied instead of nat', async () => {
		...
		const artifact_one = await (await exec(`cat src/test-data/hello-tacos.tz`)).stdout;
		await writeFile('./test-project/artifacts/hello-tacos.tz', artifact_one);
		const artifact_two = await (await exec(`cat src/test-data/mapParameter.tz`)).stdout;
	...
	test('Taqueria CLI will pass 0x00 to the simulator', async () => {
		...
		const artifact_one = await (await exec(`cat src/test-data/byteSlice.tz`)).stdout;
		await writeFile('./test-project/artifacts/byteSlice.tz', artifact_one);
		const artifact_two = await (await exec(`cat src/test-data/byteSlice.default_storage.tz`)).stdout;
		await writeFile('./test-project/artifacts/byteSlice.default_storage.tz', artifact_two);
		const artifact_three = await (await exec(`cat src/test-data/byteSlice.parameter.param1.tz`)).stdout;
	...
	test('simulator will handle Left and Right (part of input and storage value) correctly', async () => {
		...
		const artifact_one = await (await exec(`cat src/test-data/increment.tz`)).stdout;
		await writeFile('./test-project/artifacts/increment.tz', artifact_one);
		const artifact_two = await (await exec(`cat src/test-data/incrementBy3.tz`)).stdout;
