...
describe('Contract Types Plugin E2E Testing for Taqueria CLI', () => {
	...
	test('contract types plugin will offer help', async () => {
		...
		const { stdout } = await execute('taq', 'install @taqueria/plugin-contract-types@0.25.23-rc', './test-project');
	...
	test('generate types will only output the typeScriptDir and will not create a types dir when no contracts exist', async () => {
		...
		const { waitForText } = await spawn('taq', 'init test-project');
		await waitForText("Project taq'ified!");
		const { stdout, stderr } = await execute('taq', 'install @taqueria/plugin-contract-types@0.25.23-rc', './test-project');
		console.log(stderr)
		expect(stdout).toContain('Plugin installed successfully');
	...
	test('generate types types can compile one contract and generate types', async () => {
		...
		const { stdout } = await execute('taq', 'install @taqueria/plugin-contract-types@0.25.23-rc', './test-project');
		expect(stdout).toContain('Plugin installed successfully');
		const { stdout: stdout1 } = await execute('taq', 'install @taqueria/plugin-ligo@0.25.23-rc', './test-project');
